 ...
 * @author Omar Elbasha
 */
public class HorizontalLeft_1 extends State
{
    // variable to be used when moving shapes
  
    private static State Horizontal;  
     GameObject cons = (ImageObject) circus.getConstantObjects().get(4);
    
    
    private HorizontalLeft_1()
    ...
    {
        if( Horizontal == null )
            Horizontal = new HorizontalLeft_1();
    ...
    public void move(Shape shape)
    {
        ...
        if(shape.getX()<(cons.getX()))
            shape.setState(Left_Vertical_1.getInstance());
        }
    }
