 ...
 * @author Omar Elbasha
 */
public class HorizontalRight_1 extends State
{
    private static HorizontalRight_1 Horizontal = null;
    GameObject cons = (ImageObject) circus.getConstantObjects().get(3);

     public static State getInstance()
    {   
        if( Horizontal == null )
            Horizontal = new HorizontalRight_1();
    ...
    public void move(Shape shape)
     {
        ...
        if(shape.getX()>(cons.getX()+cons.getWidth()))
            shape.setState(Vertical_1.getInstance());
    }
